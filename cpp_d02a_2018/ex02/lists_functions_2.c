...
bool_t list_add_elem_at_position(list_t *front_ptr, void *elem, unsigned int
position)
{
    ...
    if (list_is_empty(*front_ptr) == TRUE || position == 0)
    ...
    if (size == position) {
        ...
        if (!new_node || size < position)
            return (FALSE);
        for (unsigned int i = 0; i < position - 2; i++, tmp = tmp->next);
        new_node->value = elem;
