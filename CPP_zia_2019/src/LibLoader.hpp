...
namespace zia
{
    ...
    template<typename T>
    std::vector<T *> LibLoader::getDirectory(const std::string &directory)
    {
    ...
    template<typename T>
    T *LibLoader::getInstance(const std::string &filepath)
    {
        T *(*fct)(void) = nullptr;
        ...
        if (libPtr != _loadedLib.end())
        {
            ...
            if (fct)
                ...
                Debug::Logger::printDebug(Debug::WARNING, "Can't find the dll entry point.", "zia::Libloader::getInstance");
        }
        else
        {
            // Getting the library.
            ...
            if (newLibrary)
            {
                ...
                Debug::Logger::printDebug(Debug::WARNING, filepath + " couldn't be loaded.", "zia::Libloader::getInstance");
        }
        return (nullptr);
    }
    ...
    template<typename T>
    T *LibLoader::getInstance(const std::string &file)
    {
        T *(*fct)(void);
        ...
        if (lib_ptr != _loadedLib.end()) {
            fct = (T *(*)())dlsym((*lib_ptr).second, LIB_ENTRYPOINT);
            if (fct != nullptr)
                return (fct());
        // Load the librairie if it is not
            ...
            if (ptr != nullptr) {
                ...
                fct = (T *(*)())dlsym(ptr, LIB_ENTRYPOINT);
                if (fct != nullptr)
                    return (fct());
            }
        }
